 ...
 * filter that compares the value of a property against the specified range.
 *
 * <ul>
...
public class PropertyIsBetweenOperatorTests extends QueryFilterFixture {
	...
	/**
	 * [{@code Test}] Submits a GetFeature request containing a {@code PropertyIsBetween}
	 * predicate that applies to some simple feature property. The response entity must
	 * include only feature instances with matching property values;
	...
	public void propertyIsBetween(ProtocolBinding binding, QName featureType) {
		try {
			...
			String xpath = String.format("xs:%s(ns1:%s) lt xs:%s('%s') and xs:%s(ns1:%s) gt xs:%s('%s')", propTypeName,
					propName.getLocalPart(), propTypeName, propValueMax, propTypeName, propName.getLocalPart(),
					propTypeName, propValueMin);
	...
	/**
	 * [{@code Test}] Submits a GetFeature request containing a PropertyIsBetween
